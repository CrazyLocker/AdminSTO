...
public class H2Database extends AbstractDatabase {
    ...
    protected void createTables(Connection conn) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(createClients)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createClientCars)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createServices)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createSpareParts)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createOrders)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createOrderServices)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createOrderParts)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createAppointments)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createServiceSpareParts)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createToParts)) { ps.execute(); }
        try (PreparedStatement ps = conn.prepareStatement(createAppSettings)) { ps.execute(); }
    ...
    private void createIndexes(Connection conn) throws SQLException {
        String[] indexes = {
                ...
                "CREATE INDEX IF NOT EXISTS idx_client_cars_client_id ON client_cars(client_id)",
        ...
        for (String index : indexes) {
            try (PreparedStatement ps = conn.prepareStatement(index)) { ps.execute(); }
        }
    }
    ...
    protected String generateOrderId(Connection conn) {
        ...
        // Генерируем формат даты для ID: dd/MM/yy (с косыми чертами, как в SQLiteDatabase)
