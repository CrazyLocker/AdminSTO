        ...
        try {
            ...
            try (ZipInputStream zis = new ZipInputStream(Files.newInputStream(backupZip))) {
                ...
                while ((entry = zis.getNextEntry()) != null) {
                    ...
                    // === Zip Slip protection (CWE-22) ===
                    // 1. Check for path traversal attempts
                    if (entryName.contains("..")) {
                        throw new SecurityException("Illegal zip entry - path traversal detected: " + entryName);
                    }
                    // 2. Check for absolute paths
                    if (entryName.startsWith("/") || entryName.startsWith("\\")) {
                        throw new SecurityException("Illegal zip entry - absolute path: " + entryName);
                    }
                    // 3. Check for Windows drive letters (e.g., "C:\\" or "C:/")
                    if (entryName.matches("^[A-Za-z]:[/\\\\].*") || entryName.contains(":/")) {
                        throw new SecurityException("Illegal zip entry - drive path: " + entryName);
                    }
                    // 4. Verify resolved path stays within target directory
                    Path basePath = Paths.get(".").toAbsolutePath().normalize();
                    Path entryPath = basePath.resolve(entryName).normalize();
                    if (!entryPath.startsWith(basePath.toString())) {
                        throw new SecurityException("Illegal zip entry - escapes target directory: " + entryName);
                    }
                    // Safe to extract
