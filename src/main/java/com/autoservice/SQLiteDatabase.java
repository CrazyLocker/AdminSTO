...
public class SQLiteDatabase extends AbstractDatabase {
    ...
    protected void createTables(Connection conn) throws SQLException {
        ...
        String createClientCars = "CREATE TABLE IF NOT EXISTS client_cars (" +
                "id INTEGER " + autoIncrement + ", " +
        ...
        String createIndexClientCars = "CREATE INDEX IF NOT EXISTS idx_client_cars_client_id ON client_cars(client_id)";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(createClients)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createClientCars)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createIndexClientCars)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createServices)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createSpareParts)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createOrders)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createOrderServices)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createOrderParts)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createAppointments)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createServiceSpareParts)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createToParts)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createAppSettings)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createServiceSparePartsLists)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createServiceSparePartsListItems)) {
            stmt.execute();
        }
        try (PreparedStatement stmt = conn.prepareStatement(createServiceParts)) {
            stmt.execute();
        }
        createIndexes(conn);
    ...
    protected String generateOrderId(Connection conn) {
        ...
        // Генерируем формат даты для ID: dd/MM/yy
    ...
    public void updateSparePart(SparePart part) {
        // Безопасно: используется PreparedStatement, конкатенация только для переноса строки
    ...
    private void migrateTables(Connection conn) throws SQLException {
        ...
        if (!columnExists(conn, "spare_parts", "unit_type")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE spare_parts ADD COLUMN unit_type TEXT DEFAULT 'шт' CHECK(unit_type IN ('шт', 'л', 'компл'))")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "spare_parts", "note")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE spare_parts ADD COLUMN note TEXT DEFAULT ''")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "services", "uses_oil")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE services ADD COLUMN uses_oil INTEGER DEFAULT 0 CHECK(uses_oil IN (0, 1))")) {
                stmt.execute();
        ...
        try (PreparedStatement stmt = conn.prepareStatement("PRAGMA table_info(spare_parts)")) {
        ...
        if (!columnExists(conn, "services", "spare_part_name")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE services ADD COLUMN spare_part_name TEXT DEFAULT ''")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "services", "spare_part_quantity")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE services ADD COLUMN spare_part_quantity INTEGER DEFAULT 0")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "to_parts", "note")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE to_parts ADD COLUMN note TEXT DEFAULT ''")) {
                stmt.execute();
        ...
        if (!tableExists(conn, "service_spare_parts_lists")) {
            try (PreparedStatement stmt = conn.prepareStatement("CREATE TABLE service_spare_parts_lists (" +
        ...
        if (!tableExists(conn, "service_spare_parts_list_items")) {
            try (PreparedStatement stmt = conn.prepareStatement("CREATE TABLE service_spare_parts_list_items (" +
                    "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    "list_id INTEGER NOT NULL CHECK(list_id > 0), " +
                    "spare_part_id INTEGER NOT NULL CHECK(spare_part_id > 0), " +
                    "quantity INTEGER DEFAULT 1 CHECK(quantity > 0), " +
        ...
        if (!columnExists(conn, "order_parts", "spare_part_id")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE order_parts ADD COLUMN spare_part_id INTEGER DEFAULT 0")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "order_parts", "unit_type")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE order_parts ADD COLUMN unit_type TEXT DEFAULT 'шт'")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "order_parts", "purchase_price")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE order_parts ADD COLUMN purchase_price REAL DEFAULT 0")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "orders", "closed_date")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE orders ADD COLUMN closed_date TEXT DEFAULT ''")) {
                stmt.execute();
        ...
        if (!columnExists(conn, "orders", "notes")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE orders ADD COLUMN notes TEXT DEFAULT ''")) {
                stmt.execute();
        ...
        if (!tableExists(conn, "service_parts")) {
            try (PreparedStatement stmt = conn.prepareStatement("CREATE TABLE service_parts (" +
                    "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    "service_id INTEGER NOT NULL CHECK(service_id > 0), " +
                    "spare_part_id INTEGER NOT NULL CHECK(spare_part_id > 0), " +
                    "quantity REAL DEFAULT 1 CHECK(quantity > 0), " +
            ...
            try (PreparedStatement stmt = conn.prepareStatement("CREATE INDEX IF NOT EXISTS idx_service_parts_service_id ON service_parts(service_id)")) {
                stmt.execute();
            }
            try (PreparedStatement stmt = conn.prepareStatement("CREATE INDEX IF NOT EXISTS idx_service_parts_spare_part_id ON service_parts(spare_part_id)")) {
                stmt.execute();
            }
        ...
        if (!columnExists(conn, "services", "category_id")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE services ADD COLUMN category_id INTEGER DEFAULT 0")) {
                stmt.execute();
                logger.info("Добавлена колонка category_id в services");
            }
        }
        
        // Добавляем колонку mileage в orders, если её нет
        if (!columnExists(conn, "orders", "mileage")) {
            try (PreparedStatement stmt = conn.prepareStatement("ALTER TABLE orders ADD COLUMN mileage INTEGER DEFAULT 0 CHECK(mileage >= 0)")) {
                stmt.execute();
                logger.info("Добавлена колонка mileage в orders");
            }
        }
        ...
        if (!tableExists(conn, "client_cars")) {
            try (PreparedStatement stmt = conn.prepareStatement("CREATE TABLE client_cars (" +
                    ...
                    ")")) {
                stmt.execute();
                logger.info("Создана таблица client_cars с CHECK constraints");
            }
            try (PreparedStatement stmt = conn.prepareStatement("CREATE INDEX IF NOT EXISTS idx_client_cars_client_id ON client_cars(client_id)")) {
                stmt.execute();
            }
        ...
        // SQLite не поддерживает ALTER TABLE DROP CONSTRAINT, поэтому пересоздаём таблицу
        migrateClientsRemoveCheckConstraints(conn);
    }
    ...
    /**
     * Миграция: удаляем CHECK-ограничения NOT NULL с car_model и car_number в таблице clients.
     * SQLite не поддерживает ALTER TABLE DROP CONSTRAINT, поэтому пересоздаём таблицу.
     */
    private void migrateClientsRemoveCheckConstraints(Connection conn) throws SQLException {
        // Проверяем, есть ли ещё CHECK на car_model через PRAGMA table_info — в SQLite
        // NOT NULL через CHECK показывается в sql, поэтому проверяем через sqlite_master
        String sql = "SELECT sql FROM sqlite_master WHERE type='table' AND name='clients'";
        try (PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
                String createSql = rs.getString("sql");
                // Если CREATE содержит CHECK(length(car_model) > 0) — нужно мигрировать
                if (createSql != null && createSql.contains("CHECK(length(car_model) > 0)")) {
                    logger.info("Миграция: удаляем CHECK constraints с car_model/car_number в clients");
                    // Пересоздаём таблицу без CHECK
                    try (PreparedStatement renameStmt = conn.prepareStatement("ALTER TABLE clients RENAME TO clients_old")) {
                        renameStmt.execute();
                    }
                    try (PreparedStatement createClientsStmt = conn.prepareStatement("CREATE TABLE clients (" +
                            "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                            "name TEXT NOT NULL CHECK(length(name) > 0), " +
                            "last_name TEXT DEFAULT '', " +
                            "phone TEXT NOT NULL CHECK(length(phone) > 0), " +
                            ...
                            "last_repair_date TEXT DEFAULT ''" +
                            ")")) {
                        createClientsStmt.execute();
                    }
                    try (PreparedStatement insertStmt = conn.prepareStatement("INSERT INTO clients (id, name, last_name, phone, car_model, car_number, last_repair_date) " +
                            "SELECT id, name, last_name, phone, car_model, car_number, last_repair_date FROM clients_old")) {
                        // Безопасно: используется PreparedStatement, конкатенация только для переноса строки
                        insertStmt.execute();
                    }
                    try (PreparedStatement dropStmt = conn.prepareStatement("DROP TABLE clients_old")) {
                        dropStmt.execute();
                    }
                    logger.info("Миграция clients завершена");
                }
            }
        }
    }
