    ...
    private static void applyFilters() {
        ...
        for (WorkOrder order : allOrders) {
            ...
            if (minTotalText != null && !minTotalText.trim().isEmpty()) {
                try { if (order.getTotal() < Double.parseDouble(minTotalText.trim())) continue; } catch (NumberFormatException ex) { logger.error("Invalid number format in order view", ex); }
            }

            if (maxTotalText != null && !maxTotalText.trim().isEmpty()) {
                try { if (order.getTotal() > Double.parseDouble(maxTotalText.trim())) continue; } catch (NumberFormatException ex) { logger.error("Invalid number format in order view", ex); }
            }
