...
public class ClientView {
    ...
    private static TableView<Client> createClientTable() {
        ...
        colPhone.setCellValueFactory(cellData -> {
            ...
            return new SimpleStringProperty(Validators.formatPhoneForDisplay(phone));
        });
        ...
        colCar.setCellValueFactory(cellData -> {
            ...
            if (cars != null && cars.size() > 1) {
                return new SimpleStringProperty(cars.size() + " авто");
            } else if (cars != null && cars.size() == 1) {
                String model = cars.get(0).getCarModel();
                return new SimpleStringProperty(model != null ? model : "—");
            } else {
                // legacy fallback
    ...
    private static void filterClients(String filterText) {
        if (filterText == null || filterText.trim().isEmpty()) {
            ...
            filteredClients.setPredicate(client -> {
                ...
                if (Validators.formatPhoneForDisplay(client.getPhone()).toLowerCase().contains(lowerFilter)) return true;
